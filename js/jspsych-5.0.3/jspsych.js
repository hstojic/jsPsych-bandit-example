...
var jsPsych = (function() {
  ...
  core.finishTrial = function(data, repeat) {
    ...
    // change the function we will use below if we repat the trial
    if (repeat) {
      var tempfnc = repeat_trial;
    } else {
      var tempfnc = next_trial;
    }

    // wait for iti
    if (typeof current_trial.timing_post_trial == 'undefined') {
      if (opts.default_iti > 0) {
        setTimeout(tempfnc, opts.default_iti);
      } else {
        tempfnc();
      }
    } else {
      if (current_trial.timing_post_trial > 0) {
        setTimeout(tempfnc, current_trial.timing_post_trial);
      } else {
        tempfnc();
      }
    }
